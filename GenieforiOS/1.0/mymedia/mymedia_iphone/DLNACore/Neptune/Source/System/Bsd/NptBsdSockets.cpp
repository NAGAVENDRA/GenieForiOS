...
#else
		if (flags & NPT_SOCKET_FLAG_CANCELLABLE) {
			m_CancelFd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
		}
#endif
    }
    ~NPT_BsdSocketFd() {
                                  ...
                                  NPT_Timeout timeout)
{
    ...
    if (m_Cancelled) {
		NPT_LOG_FINER("already cancelled, quit ASAP!");
...
#else
	if (m_SocketFdReference->m_Cancellable) {
		closesocket(m_SocketFdReference->m_CancelFd);
	}
#endif
