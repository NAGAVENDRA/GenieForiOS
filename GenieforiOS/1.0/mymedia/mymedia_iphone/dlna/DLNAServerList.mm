...
#define TABEL_HEADER_VIEW_HEIGHT 35.0f

#define REC_IS_PORTRAIT  CGRectMake(0.0f, 0.0f, 320.0f, 480.0f-TABEL_HEADER_VIEW_HEIGHT-44*2)
#define REC_IS_LANDSCAPE CGRectMake(0.0f, 0.0f, 480.0f, 320.0f-TABEL_HEADER_VIEW_HEIGHT-44-20)
...
//        m_nodeviceview.text = @"No shared device was found. To use this feature, you need to enable the file sharing feature for devices in the network, or connect a USB storage drive to your router if the router supports USB storage connection. ";
        ...
        m_nodeviceview.font = [UIFont systemFontOfSize:18];
    ...
    else
    {
        ...
        hView.backgroundColor = [UIColor colorWithRed:229.0/255 green:229.0/255 blue:251.0/255 alpha:1.0];
        hView.frame = CGRectMake(0, 0, 0, TABEL_HEADER_VIEW_HEIGHT);
...
- (void)settableHerderViewWithOrientation:(UIInterfaceOrientation)orientation
{
    ...
    {
        size = CGSizeMake(320, TABEL_HEADER_VIEW_HEIGHT);
    }
    else if (UIInterfaceOrientationIsLandscape(orientation))
    {
        size = CGSizeMake(480, TABEL_HEADER_VIEW_HEIGHT);
    }
    ...
    else
    {
        ...
        if (!isContainer)
        {
            ...
            if (mediaSize > 0)
            {
                ...
                lab.frame = CGRectMake(0, 0, 55, 38);
...
- (void) noDevice
{
    if(0 == [self.tableView numberOfRowsInSection:0])//先判断是否真的没有source设备
    {
