...
//    device.title = @"Playing";
    ...
    /*UIViewController * option = [[UIViewController alloc] init];
     option.title = @"Option";
     barItem = [[UITabBarItem alloc] initWithTitle:@"Option" image:[imgs objectAtIndex:3] tag:0];
...
//    option.title = @"Options";
    ...
    barItem = [[UITabBarItem alloc]
               initWithTitle:NSLocalizedStringFromTable(@"titleOptions", @"Localizable", nil)
               image:[imgs objectAtIndex:3] tag:0];
    option.tabBarItem = barItem;
    [barItem release];
    UINavigationController * optionNavi = [[UINavigationController alloc] initWithRootViewController:option];
    optionNavi.navigationBar.barStyle = UIBarStyleBlack;
