...
private:
	NPT_Mutex m_csWrite;
	NPT_Mutex m_csReaderCount;
	long m_cReaders;
	NPT_SharedVariable m_hevReadersCleared;
};
...
{
	m_cReaders = 0;
	m_hevReadersCleared.SetValue(1);
}

NPT_PosixRWLock::~NPT_PosixRWLock()
{
	m_hevReadersCleared.WaitUntilEquals(1);
}

NPT_Result NPT_PosixRWLock::LockForRead()
{
	m_csWrite.Lock();
	m_csReaderCount.Lock();
	if (++m_cReaders == 1) {
		m_hevReadersCleared.SetValue(0);
	}
	m_csReaderCount.Unlock();
	m_csWrite.Unlock();
	return NPT_SUCCESS;
}

NPT_Result NPT_PosixRWLock::LockForWrite()
{
	m_csWrite.Lock();
	m_hevReadersCleared.WaitUntilEquals(1);
	return NPT_SUCCESS;
...
NPT_Result NPT_PosixRWLock::UnlockForRead()
{
	m_csReaderCount.Lock();
	if (--m_cReaders == 0) {
		m_hevReadersCleared.SetValue(1);
...
NPT_Result NPT_PosixRWLock::UnlockForWrite()
{
	m_csWrite.Unlock();
	return NPT_SUCCESS;
}
