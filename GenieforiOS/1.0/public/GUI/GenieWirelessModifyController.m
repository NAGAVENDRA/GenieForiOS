//
//  GenieWirelessModifyController.m
//  GenieiPhoneiPodtest
//
...
#define TextField_SSID_FontSizeOnIphone  13
...
#define modifyChannelRowIndex           0
#define modifySecurityRowIndex          1

@implementation GenieWirelessModifyController
- (id) init
{
    self = [super init];
    if (self)
    {
        ...
        m_noSecurity = NO;
        m_isInfoChanged = NO;
        m_secutityModeInfo = [[GenieHelper getWirelessData].securityMode retain];
        m_channelInfo = [[GenieHelper getWirelessData].channel retain];
        m_target_SN_set = nil;
...
- (void)loadView
{
    ...
    self.title = Localization_Wireless_SetPage_Title;
    /////
    
    m_ssidTextField = [[UITextField alloc] initWithFrame:TextFieldFrame];
    m_ssidTextField.text = [GenieHelper getWirelessData].ssid;
...
#ifdef __GENIE_IPHONE__
    m_ssidTextField.font = [UIFont systemFontOfSize:TextField_SSID_FontSizeOnIphone];
#endif
    
    m_passwordTextField = [[UITextField alloc] initWithFrame:TextFieldFrame];
    m_passwordTextField.text = [GenieHelper getWirelessData].password;
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    if (section == 0  && !m_noSecurity)
    {
        ...
        tip.text = Localization_Wireless_Password_Length_Prompt;
...
- (void) customCell:(UITableViewCell*)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        if (index == 0)
        {
            cell.textLabel.text = Localization_Wireless_SSID_Title;
            cell.accessoryView = m_ssidTextField;
        }
        else
        {
            cell.textLabel.text = Localization_Wireless_Password_Title;
    ...
    else
    {
        if (index == 0)
        {
            cell.textLabel.text = Localization_Wireless_Channel_Title;
            cell.detailTextLabel.text = m_channelInfo;
        }
        else
        {
            cell.textLabel.text = Localization_Wireless_SecurityMode_Title;
...
#define USA_Region_Set                     [[[NSSet alloc]initWithObjects:@"us",@"united states",nil] autorelease]
...
- (void) showItemListForIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (row == modifyChannelRowIndex)
    {
        NSInteger maxChannel = 13;
        NSString * region = [[GenieHelper getWirelessData].region lowercaseString];
        if ([USA_Region_Set containsObject: region])
        {
            maxChannel = 11;
        }
        [items addObject:AutoChannel];
        for(int i = 1; i <= maxChannel; i++)
        {
            [items addObject:[NSString stringWithFormat:@"%d",i]];
        }
        
        itemListTitle = Localization_Wireless_Channel_Title;
        selectedItem = m_channelInfo;
        selector = @selector(channelChanged_Callback:);
    }
    else
    {
        ...
        itemListTitle = Localization_Wireless_SecurityMode_Title;
...
- (void) channelChanged_Callback:(NSString*) channel
{
    [m_channelInfo release];
    m_channelInfo = [channel retain];
    m_isInfoChanged = YES;
}
...
{
    GTAsyncOp * op = nil;
    NSString * region = [GenieHelper getWirelessData].region;
    NSString * wirelessMode = [GenieHelper getWirelessData].wirelessMode;
    [GenieHelper configForSetProcessOrLPCProcessStart];
    if (m_noSecurity)
    {
            op = [[GenieHelper shareGenieBusinessHelper] startSetWirelessInfoNoSecurityWithSsid:m_ssidTextField.text region:region channel:m_channelInfo wirelessMode:wirelessMode];
    }
    else
    {
        ...
        if ([m_secutityModeInfo isEqualToString:GenieSecurityModeStringWPA_PSK_WPA2_PSK])
        {
            ...
            op = [[GenieHelper shareGenieBusinessHelper] startSetWirelessInfoNeedSecurityWithSsid:m_ssidTextField.text region:region channel:m_channelInfo wirelessMode:wirelessMode securityMode:securityMode WPAPassPhrase:m_passwordTextField.text]; 
    }
    [GPWaitDialog show:op withTarget:self selector:@selector(setWirelessInfoCallback:) waitMessage:Local_WaitForSetWirelessInfo timeout:Genie_Set_Wireless_Process_Timeout cancelBtn:nil needCountDown:YES waitTillTimeout:YES];
}
...
- (void) setWirelessInfoCallback:(GenieCallbackObj*)obj
{
