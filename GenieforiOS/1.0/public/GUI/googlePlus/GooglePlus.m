...
static CGFloat shareURLTextX=20;
static CGFloat shareURLTextY=55;
static CGFloat sharePrefillTextX=20;
static CGFloat sharePrefillTextY=100;
static CGFloat sharePrefillLabelX=20;
static CGFloat sharePrefillLabelY=60;
...
-(void)gPlusShare{
    ...
    url =@"http://itunes.apple.com/cn/app/netgear-genie/id498133627?mt=8";
#else
    url=@"http://itunes.apple.com/cn/app/netgear-genie-hd-for-ipad/id498400192?mt=8";
#endif
    
    NSURL *urlToShare = [NSURL URLWithString:url];
  
    [[[[[[GenieHelper getGenieDelegate] googlePlus] shareDialog]
       setURLToShare:urlToShare]
      setPrefillText:sharePrefillText.text] open];
}
...
- (void)sizeToFitOrientation:(BOOL)transform {
    ...
    if (UIInterfaceOrientationIsLandscape(_orientation)) {
        ...
        _gView.frame= CGRectMake(kPadding, kPadding, height-kBorderWidth*2, width-kBorderWidth*2);

        sharePrefillText.frame=CGRectMake(sharePrefillTextX+20,
                                          sharePrefillTextY,
                                          sharePrefillText.frame.size.width,sharePrefillText.frame.size.height);
        
        sharePrefillLabel.frame=CGRectMake(sharePrefillLabelX+20,
                                           sharePrefillLabelY,
                                           sharePrefillLabel.frame.size.width,  sharePrefillLabel.frame.size.height);
        
        _gView.center=self.center;
    } else {
        self.frame = CGRectMake(kPadding, kPadding, width, height);

        _gView.frame= CGRectMake(kPadding, kPadding, width-kBorderWidth*2, height-kBorderWidth*2);
        _gView.center=self.center;
        
        sharePrefillText.frame=CGRectMake(sharePrefillTextX,
                                          sharePrefillTextY,
                                          sharePrefillText.frame.size.width,sharePrefillText.frame.size.height);
        
        sharePrefillLabel.frame=CGRectMake(sharePrefillLabelX,
                                           sharePrefillLabelY,
                                           sharePrefillLabel.frame.size.width,  sharePrefillLabel.frame.size.height);
    }
    self.center = center;
...
//        _delegate = nil;
        ...
        _gView=[[UIView alloc]initWithFrame:CGRectMake(kPadding, kPadding, 480, 480)];
        _gView.backgroundColor=BACKGROUNDCOLOR;
        
        shareButton = [[UIButton buttonWithType:UIButtonTypeCustom]retain];
        shareButton.frame=CGRectMake(21,209, 112, 32);
        [shareButton setBackgroundImage:[UIImage imageNamed:@"google_plus_share_large.png"] forState:UIControlStateNormal];
        [shareButton addTarget:self action:@selector(gPlusShare) forControlEvents:UIControlEventTouchUpInside];
        [_gView addSubview:shareButton];
        
//        shareUrlLabel =[[UILabel alloc]init];
//        shareUrlLabel.frame=CGRectMake(20, 26, 179, 21);
//        shareUrlLabel.text=@"URL to Share";
//        shareUrlLabel.backgroundColor=[UIColor clearColor];
//        [_gView addSubview:shareUrlLabel];
//        
//        shareURLText = [[UILabel alloc]init];
//        shareURLText.frame=CGRectMake(20, 55, 260, 31);
//        shareURLText.text=@"http://www.netgear.com/genie";
//        [_gView addSubview:shareURLText];
        
        sharePrefillLabel = [[UILabel alloc]init];
        sharePrefillLabel.frame=CGRectMake(20, 60, 156, 21);
        sharePrefillLabel.text=@"Prefill Text";
        sharePrefillLabel.backgroundColor=[UIColor clearColor];
        [_gView addSubview:sharePrefillLabel];
        
//        sharePrefillText =[[UITextField alloc]init];
//        sharePrefillText.frame=CGRectMake(20, 100, 260, 80);
        
        sharePrefillText = [[UITextView alloc]init];
        sharePrefillText.frame=CGRectMake(20, 100, 260, 80);
        sharePrefillText.layer.borderColor = [UIColor grayColor].CGColor;
        sharePrefillText.layer.borderWidth =1.0;
        sharePrefillText.layer.cornerRadius =5.0;
        
        [sharePrefillText setFont:[UIFont fontWithName:@"Helvetica" size:14]];
        sharePrefillText.text=@"Some text about share info,example:I am using netgear genie on my phone.";
#ifdef __GENIE_IPHONE__
        sharePrefillText.text=@"I'm using netgear genie for iPhone.";
#else
        sharePrefillText.text=@"I'm using netgear genie for iPad.";
#endif
        
        [_gView addSubview:sharePrefillText];


        
//        sharePrefillText.borderStyle=UITextBorderStyleRoundedRect;
//        [_gView addSubview:sharePrefillText];
...
- (void)dealloc {
    ...
    [sharePrefillLabel release];
    [sharePrefillText release];
    [shareButton release];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect webRect = CGRectMake(
                                ...
                                rect.size.width - kBorderWidth*2, _gView.frame.size.height+1);
...
//    [_modalBackgroundView setBackgroundColor:[UIColor redColor]];
...
- (void)keyboardWillShow:(NSNotification*)notification {
    ...
    if (UIInterfaceOrientationIsLandscape(orientation)) {
        ...
        [UIView animateWithDuration:0.3 animations: ^{
            sharePrefillLabel.frame=CGRectMake(sharePrefillLabelX, sharePrefillLabelY-30, sharePrefillLabel.frame.size.width, sharePrefillLabel.frame.size.height);
            
            sharePrefillText.frame=CGRectMake(sharePrefillText.frame.origin.x+sharePrefillLabel.frame.size.width, sharePrefillLabel.frame.origin.y,sharePrefillText.frame.size.width, sharePrefillText.frame.size.height);
            
            
            shareURLText.frame=
            CGRectMake(
                       sharePrefillTextX+sharePrefillLabel.frame.size.width,
                       shareUrlLabel.frame.origin.y,
                       shareURLText.frame.size.width,
                       shareURLText.frame.size.height);
        }];
...
- (void)keyboardWillHide:(NSNotification*)notification {
    ...
    if (UIInterfaceOrientationIsLandscape(orientation)) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            sharePrefillText.frame=CGRectMake(sharePrefillTextX, sharePrefillTextY, sharePrefillText.frame.size.width, sharePrefillText.frame.size.height);
            shareURLText.frame=CGRectMake(shareURLTextX, shareURLTextY, shareURLText.frame.size.width, shareURLText.frame.size.height);
            
            sharePrefillLabel.frame=CGRectMake(sharePrefillLabelX, sharePrefillLabelY, sharePrefillLabel.frame.size.width, sharePrefillLabel.frame.size.height);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (touch.tapCount >=1) {
        [sharePrefillText resignFirstResponder];
        [shareURLText resignFirstResponder];
...
- (void)dialogDidSucceed:(NSURL *)url {
    NSLog(@"%@",[url description]);
//    if (![[url description] isEqualToString:@"fbconnect://success"]) {
//        
