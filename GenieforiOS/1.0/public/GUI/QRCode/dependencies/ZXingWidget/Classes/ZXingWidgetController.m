...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode {
  ...
  if (self) {
    ...
    beepSound = -1;
    decoding = NO;
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width/4*3 , [UIScreen mainScreen].bounds.size.height/5*4)
                                                       cancelEnabled:showCancel 
                                                            oneDMode:oneDMode];
      
      theOverLayView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height/2);
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
  ...
  [self performSelector:@selector(notifyDelegate:) withObject:[[twoDResult text] copy] ];
       // remove 'afterDelay' by gky
    
    [self performSelector:@selector(notifyDelegateImage:) withObject:[image copy] ];
...
-(void)notifyDelegateImage:(id)image{
    [delegate zxingController:self didScanResultImage:image];
    [image release];
}
...
//    if (decoding) {
//    UIImage *ref = [[UIImage alloc] initWithCGImage: newImage scale: 1.0f orientation: UIImageOrientationRight];
//    captureImage = [ref retain];
////        captureImage = scrn ;
////        captureImage = ref;
//        [ref release];
//    }
